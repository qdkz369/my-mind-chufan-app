...
export async function GET(request: Request) {
  try {
    ...
    // 应用状态筛选
    if (status) {
      repairs = repairs.filter((order: any) => order.status === status.toLowerCase())
      console.log("[报修列表API] 状态筛选后数量:", repairs.length)
    }

    // 应用餐厅筛选
    if (restaurantId) {
      repairs = repairs.filter((order: any) => order.restaurant_id === restaurantId)
    ...
    if (workerId) {
      ...
      if (status && status !== "all" && status !== "pending") {
        ...
        repairs = repairs.filter((order: any) => order.assigned_to === workerId)
