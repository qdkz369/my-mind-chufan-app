...
export type OrderFactContract = {
  order_id: string  // 数据来源：delivery_orders.id
  restaurant_id: string  // 数据来源：delivery_orders.restaurant_id
  status: string  // 数据来源：delivery_orders.status
  created_at: string  // 数据来源：delivery_orders.created_at
  accepted_at: string | null  // 数据来源：audit_logs.created_at（当 action = ORDER_ACCEPTED）
  completed_at: string | null  // 数据来源：audit_logs.created_at（当 action = ORDER_COMPLETED）
  worker_id: string | null  // 数据来源：delivery_orders.worker_id
}

export type TraceFactContract = {
  id: string  // 数据来源：trace_logs.id
  asset_id: string  // 数据来源：trace_logs.asset_id
  action_type: '出厂' | '充装' | '配送' | '回收' | '安检'  // 数据来源：trace_logs.action_type
  operator_id: string  // 数据来源：trace_logs.operator_id
  order_id: string | null  // 数据来源：trace_logs.order_id
  created_at: string  // 数据来源：trace_logs.created_at
}

export type AssetFactContract = {
  asset_id: string  // 数据来源：gas_cylinders.id 或 devices.device_id
  status: string  // 数据来源：gas_cylinders.status 或 devices.status
  last_action: string  // 数据来源：trace_logs.action_type（最后一次操作）
  last_action_at: string  // 数据来源：trace_logs.created_at 或 gas_cylinders.updated_at
}
...
**修改位置：**
- 第 27 行：导入 `OrderFactContract`、`TraceFactContract`、`AssetFactContract`
- 第 179-187 行：使用 `OrderFactContract` 类型，`accepted_at` 和 `completed_at` 使用 `null` 而不是 `undefined`
- 第 204-211 行：使用 `TraceFactContract[]` 类型，`order_id` 使用 `null` 而不是 `undefined`
- 第 256-261 行、第 264-269 行：使用 `AssetFactContract` 类型
...
// 修改后
accepted_at: acceptedAt || null  // 使用 null 表示事实不存在
```
...
**文件路径：** `app/api/facts/restaurant/[restaurant_id]/assets/route.ts`

**修改位置：**
- 第 25 行：导入 `AssetFactContract`
- 第 97-110 行：使用 `AssetFactContract` 类型
