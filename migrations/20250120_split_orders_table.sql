...
-- 报修工单与燃料配送表拆分迁移脚本
-- 创建日期: 2025-01-20
-- 说明: 将 orders 表拆分为 repair_orders 和 delivery_orders
-- ============================================
...
-- 3.1 迁移报修工单数据（处理 user_id 和 notes 字段可能不存在的情况）
DO $$
DECLARE
  has_user_id BOOLEAN;
  has_notes BOOLEAN;
BEGIN
  ...
  IF has_user_id AND has_notes THEN
    -- orders 表有 user_id 和 notes 字段，正常迁移
    EXECUTE '
    ...
    SELECT 
      id, NULLIF(restaurant_id, '''')::UUID, NULLIF(user_id, '''')::UUID, service_type, status, description,
      audio_url, device_id, urgency, assigned_to, worker_id,
    ...
    RAISE NOTICE '报修工单数据迁移完成（包含 user_id 和 notes）';
  ELSIF has_user_id AND NOT has_notes THEN
    -- orders 表有 user_id 但没有 notes 字段
    EXECUTE '
    INSERT INTO repair_orders (
      id, restaurant_id, user_id, service_type, status, description,
      audio_url, device_id, urgency, assigned_to, worker_id,
    ...
    SELECT 
      id, NULLIF(restaurant_id, '''')::UUID, NULLIF(user_id, '''')::UUID, service_type, status, description,
      audio_url, device_id, urgency, assigned_to, worker_id,
      COALESCE(amount, 0), COALESCE(customer_confirmed, false), NULL::TEXT,
      created_at, updated_at
    FROM orders
    WHERE service_type IN (''维修服务'', ''清洁服务'', ''工程改造'')
      AND id NOT IN (SELECT id FROM repair_orders)
    ...
    RAISE NOTICE '报修工单数据迁移完成（包含 user_id，notes 使用 NULL）';
  ELSIF NOT has_user_id AND has_notes THEN
    -- orders 表没有 user_id 但有 notes 字段
    EXECUTE '
    INSERT INTO repair_orders (
      id, restaurant_id, user_id, service_type, status, description,
      audio_url, device_id, urgency, assigned_to, worker_id,
    ...
    SELECT 
      id, NULLIF(restaurant_id, '''')::UUID, NULL::UUID, service_type, status, description,
      audio_url, device_id, urgency, assigned_to, worker_id,
      COALESCE(amount, 0), COALESCE(customer_confirmed, false), notes,
      created_at, updated_at
    FROM orders
    ...
    RAISE NOTICE '报修工单数据迁移完成（user_id 使用 NULL，包含 notes）';
  ELSE
    -- orders 表既没有 user_id 也没有 notes 字段
    EXECUTE '
    INSERT INTO repair_orders (
      id, restaurant_id, user_id, service_type, status, description,
      audio_url, device_id, urgency, assigned_to, worker_id,
    ...
    RAISE NOTICE '报修工单数据迁移完成（user_id 和 notes 都使用 NULL）';
  END IF;
END $$;

-- 3.2 迁移燃料配送订单数据（处理 user_id 和 notes 字段可能不存在的情况）
DO $$
DECLARE
  has_user_id BOOLEAN;
  has_notes BOOLEAN;
BEGIN
  ...
  IF has_user_id AND has_notes THEN
    ...
    SELECT 
      id, NULLIF(restaurant_id, '''')::UUID, NULLIF(user_id, '''')::UUID, product_type, service_type, status,
      assigned_to, worker_id, tracking_code, proof_image,
      COALESCE(amount, 0), COALESCE(customer_confirmed, false), notes,
      created_at, updated_at
    FROM orders
    ...
    RAISE NOTICE '配送订单数据迁移完成（包含 user_id 和 notes）';
  ELSIF has_user_id AND NOT has_notes THEN
    -- orders 表有 user_id 但没有 notes 字段
    EXECUTE '
    INSERT INTO delivery_orders (
    ...
    RAISE NOTICE '配送订单数据迁移完成（包含 user_id，notes 使用 NULL）';
  ELSIF NOT has_user_id AND has_notes THEN
    -- orders 表没有 user_id 但有 notes 字段
    EXECUTE '
    INSERT INTO delivery_orders (
    ...
    SELECT 
      id, NULLIF(restaurant_id, '''')::UUID, NULL::UUID, product_type, service_type, status,
      assigned_to, worker_id, tracking_code, proof_image,
      COALESCE(amount, 0), COALESCE(customer_confirmed, false), notes,
      created_at, updated_at
    FROM orders
    ...
    RAISE NOTICE '配送订单数据迁移完成（user_id 使用 NULL，包含 notes）';
  ELSE
    -- orders 表既没有 user_id 也没有 notes 字段
    EXECUTE '
    INSERT INTO delivery_orders (
    ...
    RAISE NOTICE '配送订单数据迁移完成（user_id 和 notes 都使用 NULL）';
  END IF;
END $$;
