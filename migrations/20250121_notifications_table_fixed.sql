-- ============================================
-- 通知管理表创建脚本（修复版 - 兼容 TEXT 和 UUID）
-- 创建日期: 2025-01-21
-- 说明: 用于管理客户端通知消息（订单通知、系统通知等）
-- 修复: restaurant_id 支持 TEXT 类型以兼容现有 restaurants 表
-- ============================================
...
-- 创建 notifications 表
-- 注意：restaurant_id 使用 TEXT 类型以兼容现有 restaurants 表（可能是 TEXT 或 UUID）
CREATE TABLE IF NOT EXISTS notifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  
  -- 接收者信息（使用 TEXT 以兼容 UUID 和 TEXT 类型的 restaurant_id）
  restaurant_id TEXT NOT NULL,                        -- 餐厅ID（接收通知的餐厅，兼容 TEXT 和 UUID）
